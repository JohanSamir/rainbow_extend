...
env_inf = {
    "CartPole": {
        "MIN_VALS": jnp.array([-2.4, -5., -math.pi / 12., -math.pi * 2.]),
        "MAX_VALS": jnp.array([2.4, 5., math.pi / 12., math.pi * 2.])
    },
    "Acrobot": {
        "MIN_VALS": jnp.array([-1., -1., -1., -1., -5., -5.]),
        "MAX_VALS": jnp.array([1., 1., 1., 1., 5., 5.])
    },
    "MountainCar": {
        "MIN_VALS": jnp.array([-1.2, -0.07]),
        "MAX_VALS": jnp.array([0.6, 0.07])
    }
}
...
class ImplicitQuantileNetwork(nn.Module):
    ...
    def __call__(self, x, num_quantiles, rng):
        ...
        quantile_net = nn.Dense(features=state_vector_length, kernel_init=self.initzer)(quantile_net)
