...
flags.DEFINE_integer('replay_capacity',
                     50000,
...
flags.DEFINE_integer('num_runs',
                     ...
                     lower_bound=1)
flags.DEFINE_string('log_root', "../../test_joao/offline/",
                    "the root of your loggin dir")
    ...
    for agent in ags:
        ...
        for i in range(FLAGS.num_runs):
            LOG_PATH = os.path.join(path,
                                    FLAGS.log_root + f'{agent}/online_{i+1}')

            gin_file = f'./Configs/{agent}_cartpole.gin'
            gin.clear_config()
            gin.parse_config_file(gin_file)
            gin.bind_parameter(
                f"OutOfGraphPrioritizedReplayBuffer.replay_capacity",
            ...
            LOG_PATH = os.path.join(path,
                                    FLAGS.log_root + f'{agent}/fixed_{i+1}')
