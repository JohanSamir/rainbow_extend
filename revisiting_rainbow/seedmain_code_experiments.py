        ...
        for env in environments:
            for init in inits:
                for i in range(1, num_runs + 1):
                    ...
                    LOG_PATH = os.path.join(f'{path}{agent}_{env}', f'test{i}')
                    sys.path.append(path)
                    gin_file = f'Configs/{agent}_{env}.gin'

                    if init == 'zeros' or init == 'ones':
                        gin_bindings = [f"{agent_name}.seed=None"] if seed is False else [
                            f"{agent_name}.seed={i}", f"{agent_name}.initzer = @{initializer}"
                        ]
                        ...
                        gin_bindings = [f"{agent_name}.seed=None"] if seed is False else [
                            f"{agent_name}.seed={i}", f"{agent_name}.initzer = @{initializer}()",
                            f"{initializer}.scale = 1", f"{initializer}.mode = {mode}",
