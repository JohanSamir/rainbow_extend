...
def target_distributionDouble(model, target_network, next_states, rewards,
...
def train(network_def, target_params, optimizer, states, actions, next_states,
          rewards, terminals, loss_weights, kappa, num_atoms, cumulative_gamma,
          double_dqn, rng):
    """Run a training step."""
        ...
        if self._replay.add_count > self.min_replay_history:
            if self.training_steps % self.update_period == 0:
                ...
                self.optimizer, loss, mean_loss = train(
                    ...
                    self._kappa, self._num_atoms, self.cumulative_gamma,
